...
npm test           # combat-engine, 3D data-integrity and Roblox export tests (node --test)
npm run export:rpgmaker   # build/rpgmaker/*.json for an RPG Maker MZ project
npm run export:roblox     # roblox/src/shared/Data/*.luau from the 3D build (see roblox/README.md)
```
...
| `roblox/` | The Roblox rebuild: a Rojo project with the 3D build's data exported to Luau, the combat rules ported and tested under Lune, and a playable first slice (select, waves, 16 technique types, HUD, web-swing). See `roblox/README.md`. |
